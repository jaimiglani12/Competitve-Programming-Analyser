...
def dif_analysis(handle):
    ...
    rating_sum=0
    problem_count=0
    # df=pd.DataFrame(submissions["problem"]["rating"])
    easy=0
    hard=0
    medium=0
    for sub in submissions:
...
def insights_analysis(handle):
    ...
    if weak_topics:
        insights.append(
            f"You should practice more {', '.join(weak_topics[:3])} problems."
        )
    ...
    if avg_rating:
        ...
        insights.append(
            f"Recommended next goal: solve problems around {next_target} rating."
        )
...
def contest_analysis(handle):
    ...
    for contest in contests:
        ...
        if rank:
            ranking.append(rank)
        if oldRating is not None and new_rating is not None:
            delta=new_rating-oldRating
            rating.append(delta)
